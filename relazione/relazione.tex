...
\end{itemize}
Questi indicatori indicano che la scelta ottimale è \textbf{k=2}, in quanto garantisce la massima separazione e coesione dei gruppi, fornendo al contempo profili commerciali chiari e facilmente interpretabili per le strategie di business.
...
La Figura \ref{fig:decision_tree_vis} mostra le regole logiche dell'albero di decisione. Ciascun nodo contiene le seguenti informazioni:
\begin{itemize}
    \item \textbf{Condizione in alto} (es. `Contract\_Month-to-month $\le 0.5$'): La regola di split. Se il cliente rispetta la condizione si scende a sinistra (SI), altrimenti a destra (NO).
    \item \textbf{gini}: L'impurezza del nodo (0 indica massima purezza, ovvero che tutti i clienti in quel nodo appartengono alla stessa classe; 0.5 indica massima incertezza).
    \item \textbf{samples}: Il numero di clienti che cadono in quel nodo.
    \item \textbf{value}: La distribuzione delle classi, espressa come \texttt{[No Churn, Churn]}.
    \item \textbf{class}: La classe di maggioranza assegnata provvisoriamente.
    \item \textbf{Colori}: I nodi sono colorati in \textbf{blu} se la classe di maggioranza è No Churn (classe 0) e in \textbf{arancione} se è Churn (classe 1). L'intensità del colore indica il grado di purezza (nodi blu scuro o arancione scuro sono molto puri, mentre quelli bianchi indicano incertezza vicina al 50/50).
\end{itemize}
...
La Figura \ref{fig:feature_importances} mostra le 10 caratteristiche più importanti per l'albero di decisione nello Scenario B:
\begin{itemize}
    \item La variabile determinante in assoluto è `Contract\_Month-to-month` (avere un contratto mensile flessibile).
    \item La seconda variabile è la `tenure` (i mesi di permanenza attiva).
    \item Al terzo posto c'è `InternetService\_Fiber optic` (internet in fibra, legato a spese mensili elevate).
    \item Il nostro `Cluster\_1` (profilo ad alta spesa volatile) compare nella top 10 delle feature importanti. Questo convalida quantitativamente l'utilità del clustering non supervisionato.
\end{itemize}
